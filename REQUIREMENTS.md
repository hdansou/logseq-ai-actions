...
- The asset-byte loader (`src/adapter/image-loader.ts`) tries `fetch(url)` first; on failure, falls back to `<img>` → canvas → `toDataURL(mimeType)`. The fetch path is the primary working path in current Logseq Desktop — `makeUrl` returns `file:///abs/path`, and the plugin iframe's runtime is permissive enough to read it even from an `http://localhost` HMR origin (Logseq's plugin host effectively disables `webSecurity` for the iframe; this is empirically verified, not documented in the SDK). On Logseq Web, `makeUrl` returns a `blob:` URL and fetch works directly. The canvas path is a defensive fallback — kept because (1) we have observed at least one run where fetch was blocked before a clean plugin re-enable cycle, and (2) it costs almost nothing to keep. Re-encoding via `toDataURL` is lossless for PNG and acceptable for JPEG/WebP (vision models downscale anyway).
- Returns a discriminated `{ ok: true, … } | { ok: false, reason, hint? }` (reasons: `no-path`, `no-type`, `unsupported-mime`, `makeurl-failed`, `fetch-failed`, `decode-failed`) so `runVisionAction`'s toast names the actual failure instead of the generic "path or asset type unrecognised". Pure `describeOriginMismatch(origin, url)` adds a last-resort hint when *both* fetch and canvas fail and the plugin is on an HTTP origin reading a `file://` URL — covers the edge case where a future Logseq build tightens its iframe policy. There is no SDK route to read asset bytes directly — `IAssetsProxy` exposes only `makeUrl`, `listFilesOfCurrentGraph`, `makeSandboxStorage`, and `builtInOpen`, none of which returns bytes.
