...
- The asset-byte loader (`src/adapter/image-loader.ts`) routes through Logseq's postMessage caller via `logseq._execCallableAPIAsync("exper_request", pluginId, { url, method: "GET", returnType: "base64" })`. `Assets.makeUrl` returns `file:///abs/path` on Electron (`logseq/src/main/frontend/handler/assets.cljs:154`), and the plugin iframe runs at the `lsp://logseq.io/...` origin cross-origin with the Logseq main window, so `fetch(file://)` and `<img src="file://">` both hit Chromium's "Not allowed to load local resource". `logseq.Request._request` is *also* unavailable in this configuration: it uses `Experiments.invokeExperMethod` → `ensureHostScope()` which synchronously reads `parent.window.logseq`, and that property access throws `SecurityError` cross-origin. The Postmate-based caller (`logseq/libs/src/LSPlugin.caller.ts`) uses `window.postMessage` which is cross-origin-safe; calling `exper_request` through it triggers the same `:httpRequest` IPC handler that `Request._request` would (`logseq/src/electron/electron/handler.cljs:353-389`, using `node-fetch` which supports `file://`), but without the synchronous parent-property access. The host's `request-callback` (`logseq/src/main/frontend/handler/plugin.cljs:890`) returns the base64 payload via the `#lsp#request#callback` postMessage event. Renderer-side `fetch` and `<img>+canvas` are kept as fallbacks for Logseq Web.
