...
- `runAction` now branches on `action.outputMode` with three paths: `replace` (direct updateBlock), `diff-panel` (DiffPanel, updateBlock on accept), `append-children` (ConfirmPanel → sequential `logseq.Editor.insertBlock` with `sibling: false` so each parsed line becomes a child block). Emits a tagged-union stream (`same` / `added` / `removed`) with empty-value segments dropped. Round-trip invariant: concatenating `same + added` reconstructs proposed; `same + removed` reconstructs original. 6 tests.
