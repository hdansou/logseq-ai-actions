...
- **`logseq.Assets.makeUrl` returns `file:///abs/path` on Desktop, `blob:` on Web.** Vision works in `pnpm dev` HMR — the plugin iframe in current Logseq Desktop is permissive enough to `fetch(file://)` from `http://localhost` (empirical, not documented in the SDK). The asset-byte loader (`src/adapter/image-loader.ts`) tries fetch first and falls back to `<img>` → canvas → `toDataURL` if fetch ever fails. There is no SDK route to read bytes directly (`IAssetsProxy` exposes `makeUrl`/`listFilesOfCurrentGraph`/`makeSandboxStorage`/`builtInOpen`; none returns bytes), so if Logseq tightens iframe policy in a future build the canvas fallback is what we have. `describeOriginMismatch` (`src/asset-url.ts`) emits a last-resort hint pointing at `pnpm build:watch` if both paths fail.
