...
- **Loading raw bytes for an asset path: postMessage `exper_request` IPC, not `fetch`.** `logseq.Assets.makeUrl` returns `file:///abs/path` on Desktop, `blob:` on Web. From the plugin iframe `fetch(file://)` and `<img src="file://">` are blocked by Chromium. `Request._request` is broken (see above). The working path is `logseq._execCallableAPIAsync("exper_request", pluginId, { url, method: "GET", returnType: "base64" })`, listening for `#lsp#request#callback` events on `logseq.caller`. The host's `:exper_request` (`logseq/src/main/logseq/api.cljs:168`) fires the same `:httpRequest` IPC handler `Request._request` would have — base64-encoded server-side via `node-fetch` (handles `file://`). Race protection: register the listener BEFORE calling `_execCallableAPIAsync` and buffer responses by `requestId` until you know which is yours. See `src/adapter/image-loader.ts:tryPostmateExperRequestBase64`.
- **The SDK `.d.ts` understates what's available — read Logseq's cljs source instead.** v1.0.1 shipped fetch + `<img>+canvas` assuming `IAssetsProxy` was the only file-I/O surface. v1.0.2 used `Request._request` assuming it'd work everywhere. v1.0.3 added diagnostics. v1.0.4 finally found the postMessage caller path. Each iteration would have been avoided by reading `/Users/dzu/Projects/src/github.com/logseq-dev/logseq/libs/src/LSPlugin.caller.ts` and `.../src/main/logseq/api.cljs` upfront. Grep cljs before designing around an apparent SDK limitation.
